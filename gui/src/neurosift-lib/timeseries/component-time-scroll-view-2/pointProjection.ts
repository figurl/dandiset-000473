 ...
 */
export const convert1dDataSeries = (
  data: number[],
  transform: Matrix,
): number[] => {
 ...
 */
export const use1dScalingMatrix = (
  pixelWidth: number,
  dataStart: number | undefined,
  dataEnd: number | undefined,
  extraPixelOffset: number = 0,
) => {
  return useMemo(() => {
    if (dataStart === undefined || dataEnd === undefined) {
      console.warn(
        "Attempt to compute 1d scaling matrix, but either start or length is unset. Mapping to null.",
      );
      return matrix([0, 0]);
    }
    if (dataStart === dataEnd) {
      console.warn(
        "Attempt to compute 1d scaling matrix with 0-length data. Mapping to null.",
      );
      return matrix([0, 0]);
    }
    ...
    return matrix([
      pixelsPerBaseUnit,
      extraPixelOffset + dataStart * -pixelsPerBaseUnit,
    ]);
 ...
 */
export const usePointWidthsFromIntervals = (
  ...
  return (
    dataInPairs
      ? transpose(pixelStartsAndWidths).valueOf()
      : pixelStartsAndWidths.valueOf()
  ) as number[][];
};

const applyPixelMargins = (
  totalPixelWidth: number,
  totalPixelHeight: number,
  pixelMargins?: PartialMargins,
) => {
...
const handleAspectRatio = (pixelDims: Dims, dataDims: Dims): Dims => {
  ...
  if (aspectRatioPx > aspectRatioD) {
    ...
    return {
      ...pixelDims,
      xMin: pixelDims.xMin + extraWidth / 2,
      xMax: pixelDims.xMax - extraWidth / 2,
    };
  } else {
    ...
    return {
      ...pixelDims,
      yMin: pixelDims.yMin + extraHeight / 2,
      yMax: pixelDims.yMax - extraHeight / 2,
    };
  }
};
...
const computeScales = (
  ...
  const adjustedPixelDims = preserveAspectRatio
    ? handleAspectRatio(pixelDims, dataDims)
    : pixelDims;
  ...
  const xScale = xRangeD === 0 ? 0 : xRangePx / xRangeD;
  const yScale = yRangeD === 0 ? 0 : yRangePx / yRangeD;

  const xOffsetPx = adjustedPixelDims.xMin + -xScale * dataDims.xMin;
  const yOffsetPx =
    adjustedPixelDims.yMin +
    (invertY ? yScale * dataDims.yMax : -yScale * dataDims.yMin);
  return { xScale, yScale, xOffsetPx, yOffsetPx };
};

const matrixFromScaleOffset = (
  scale: scaleOffset,
  xScaleFactor: number,
  yScaleFactor: number,
) => {
...
export const use2dScalingMatrix = (props: ScalingProps) => {
  ...
  const { dataXMin, dataXMax, dataYMin, dataYMax, yScaleFactor, xScaleFactor } =
    props;
  const xformMatrix = useMemo(() => {
    ...
    if (
      ...
      console.warn(
        "Attempt to compute pixel projection matrix with undefined data elements. Mapping to null.",
      );
      return matrix([
        [0, 0, 0],
        [0, 0, 0],
    ...
    if (dataXMin === dataXMax && dataYMin === dataYMax) {
      console.warn(
        "Attempt to compute pixel projection matrix, but data has 0 width in both dimensions. Mapping to null.",
      );
      return matrix([
        [0, 0, 0],
        [0, 0, 0],
    ...
    const _pixelDims = applyPixelMargins(
      totalPixelWidth,
      totalPixelHeight,
      pixelMargins,
    );
    ...
    const xform = matrixFromScaleOffset(
      scale,
      xScaleFactor ?? 1,
      (yScaleFactor ?? 1) * (invert ? -1 : 1),
    );
    return xform;
  }, [
    positiveYGrowsDownward,
    preserveAspectRatio,
    dataXMin,
    dataXMax,
    dataYMin,
...
export const getYAxisPixelZero = (transformMatrix2d: Matrix) => {
  ...
  const value = (
    multiply(transformMatrix2d, augmentedZero).valueOf() as number[][]
  )[1][0];
  return value;
};
 ...
 */
export const convert2dDataSeries = (
  transform: Matrix,
  data: number[][],
  dataAsPairs?: boolean,
) => {
  ...
  return (
    dataAsPairs
      ? transpose(convertedSeries).valueOf()
      : convertedSeries.valueOf()
  ) as number[][];
};
 ...
 */
export const convertBaseDistanceToPixelDistance = (
  xDistance: number,
  yDistance: number,
  transform: Matrix,
) => {
